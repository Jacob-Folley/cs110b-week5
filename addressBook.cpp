...
void sortArray(Person persons[], int length) {
	...
	for (i = 0; i < length - 1; i++) {
		...
		for (j = i + 1; j < length; j++) {
			persons[j].firstName[0] = toupper(persons[j].firstName[0]);
			persons[j].lastName[0] = toupper(persons[j].lastName[0]);
...
void printAddressBook(Person persons[], int numberOfPeople) {
	...
	for (int i = 0; i < numberOfPeople; i++) {
		cout << i + 1 << '.' << endl;
		persons[i].firstName[0] = toupper(persons[i].firstName[0]);
		persons[i].lastName[0] = toupper(persons[i].lastName[0]);
		cout << persons[i].name() << endl;
		cout << persons[i].telephoneNumber << endl;
		cout << persons[i].streetAddress << endl;
		cout << persons[i].email << endl;
			...
			if (fname.compare(persons[i].firstName) == 0) {
				...
				case 1: 
					...
					if (deleteChoice == 'Y' || 'y') {
						...
						cout << "Enter telephone number: ";
						cin >> persons[i].telephoneNumber;
						cout << "Enter street address: ";
						cin >> persons[i].streetAddress;
						cout << "Enter email: ";
						cin >> persons[i].email;
						cout << endl;
					}
				...
				case 2: // Delete
					...
					if (deleteChoice == 'Y' || 'y') {
						...
						for (int j = i; j < (numberOfPeople - i); j++) {
							persons[j].firstName = persons[j + 1].firstName; 
							persons[j].lastName = persons[j + 1].lastName;
							persons[j].telephoneNumber = persons[j + 1].telephoneNumber;
							persons[j].streetAddress = persons[j + 1].streetAddress;
							persons[j].email = persons[j + 1].email;
						}
